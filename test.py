...
path_of_car = r'Images\1.jpg'#path of car to detection car and license plate recognition
...
def Alarm_system():
    ser = serial.Serial(port=arduino_nano_port, baudrate=9600, timeout= 0.2)
    ser.reset_input_buffer()
    while True:
        ser.write(MODE)
...
def main():
    ...
    while True:
        ...
        if flag_detect_car == True:
            (flag_my_license_plate, foreign_car_license_plate) = License_plate_recognition()
            if flag_my_license_plate == False:
                print("A foreign car is parked in your private parking lot with its number:", foreign_car_license_plate)
                Send_Image()
                response_from_server = Dialogue(foreign_car_license_plate)
